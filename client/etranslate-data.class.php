...
abstract class eTranslateData {
	public $allow_cache = true;

	protected function isCacheValid( $cache_file, $max_age = false ) {
		if( !$this->allow_cache ) {
			$this->why_no_cache = "Cache not allowed";
			return false;
		}
		elseif( !file_exists( $cache_file ) ) {
			$this->why_no_cache = "No cache file $this->cache_file";
			return false;
		}
		elseif( filesize( $cache_file ) < 10 ) {
			$this->why_no_cache = "Cache file too small";
			return false;
		}
		elseif( !file_get_contents( $cache_file ) ) {
			$this->why_no_cache = "Cache file not readable";
			return false;
		}
		return true;
	}
	...
	protected function split_url( $url, $decode=TRUE ) {
