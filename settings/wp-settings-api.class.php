		...
		<tr valign="top">
			...
			<td class="forminp">
				<fieldset>
					...
					if( $stored_value === false && isset( $data['default'] ) ) {
					...
					if( isset( $data['values'] ) ) foreach( $data['values'] as $value => $label ) :
					 ...
					 ?>
					<input <?php disabled( $data['disabled'], true ); ?> class="<?php echo esc_attr( $data['class'] ); ?>" type="radio" name="<?php echo esc_attr( $field_key ); ?>" id="<?php echo esc_attr( $field_key ) . '_' . sanitize_title( $value );?>" value="<?php echo $value; ?>" style="<?php echo esc_attr( $data['css'] ); ?>" value="<?php echo $value ?>" <?php checked( $stored_value, $value ); ?> <?php echo $this->get_custom_attribute_html( $data ); // WPCS: XSS ok. ?> />
